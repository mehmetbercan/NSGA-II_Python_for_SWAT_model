...
#-------------------------------------------------------------------------------
# Name:        NSGA-II
# Purpose:     NSGA-II utilities for pynsga2 (non-dominating sorting and crowding distance functions)
#
...
def CreatePopulationDataframe(pop, parnames):
    popsize = len(pop["ind"])

    index = []
    par_fitnes_dic ={}
    for p in range(len(parnames)):
        par_fitnes_dic[parnames[p]] = []
    for f in range(len(pop['ind'][0]['fitness'])):
        par_fitnes_dic['f{}'.format(f+1)] = []

    for i in range(popsize):
        index.append(i)
        params = pop['ind'][i]['xbin']
        finesses = pop['ind'][i]['fitness']
        for p in range(len(params)):
            par_fitnes_dic[parnames[p]].append(params[p])
        for f in range(len(finesses)):
            par_fitnes_dic['f{}'.format(f+1)].append(finesses[f])
        
    return pd.DataFrame(par_fitnes_dic, index=index)
...
def Selection(old_pop_ptr,pop2_ptr,warmup_random): #nselect() in deb's c code #select solutions from old population to the mate population. All members on mate population updated.
    ...
    for n in range(0,popsize):
        ...
        if(rnd1 == popsize):rnd1 = int(abs(popsize - 4)/2);
...
def NonDominatedSorting(gen,popsize,global_pop_ptr,nfunc,globalpop,OutDir):#grank
    gr = open(os.path.join(OutDir, "NSGA2.Out", "g_rank_record.out"),"a");
...
def CreateMatePopFromNewandOldPops(pop1_ptr,pop2_ptr,pop3_ptr,gen,OutDir): #keepaliven
    ...
    NonDominatedSorting(gen,popsize,global_pop_ptr,nfunc,globalpop,OutDir);
